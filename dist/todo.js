...
const getTask = async () => {
    if (tasks.length >= 1) {
        ...
        tasks.forEach((currElem) => {
            console.log(chalk.bold(`${currElem.taskId} \t ${currElem.createDate.getDate()} / ${currElem.createDate.getMonth() + 1} / ${currElem.createDate.getFullYear()} \t ${currElem.taskName} `));
        });
        console.log(" ");
    }
    else {
...
const startProgramme = async () => {
    ...
    while (!keepContinue) {
        ...
        else if (x.includes("Add")) {
            await addTodo();
            console.log(chalk.yellow("Todo added"));
        }
        else if (x.includes("List")) {
            await getTask();
        }
