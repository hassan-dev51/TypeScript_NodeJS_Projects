...
console.log(
  chalk.bold(`IN THIS GAME YOU HAVE 3 CHANCE TO FIND THE HIDDEN NUMBER .\n\n `)
);
...
async function restartGame() {
  ...
  if (
    ask_user.restart.toLowerCase() == "y" ||
    ask_user.restart.toLowerCase() == "yes"
  ) {
    startGame();
  } else {
    console.log(
      chalk.cyanBright.bold(`Hope you enjoyed the game! ${player_name.user}`)
    );
  }
}
startGame();
